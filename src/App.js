...
function App() {
  ...
  const [darkMode, setDarkMode] = useState(() => {
    return localStorage.getItem('cinehub-theme') === 'dark';
  });

  // Track body class updates when theme changes
  useEffect(() => {
    if (darkMode) {
      document.body.classList.add('dark-theme');
      localStorage.setItem('cinehub-theme', 'dark');
    } else {
      document.body.classList.remove('dark-theme');
      localStorage.setItem('cinehub-theme', 'light');
    }
  }, [darkMode]);
  ...
  return (
    <div className="app">
      {/* Decorative background glow blobs for dark mode glassmorphism */}
      <div className="glass-blob blob-1" aria-hidden="true"></div>
      <div className="glass-blob blob-2" aria-hidden="true"></div>
      <div className="glass-blob blob-3" aria-hidden="true"></div>
